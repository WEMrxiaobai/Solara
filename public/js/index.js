...
function buildAudioProxyUrl(url, forceProxy = false) {
    if (!url || typeof url !== "string") return url;

    try {
        const parsedUrl = new URL(url, window.location.href);
        // 如果是 kuwo.cn 域名，且是 http 协议或强制代理，则使用代理
        if (/(^|\.)kuwo\.cn$/i.test(parsedUrl.hostname)) {
            if (parsedUrl.protocol === "http:" || forceProxy) {
...
async function downloadSong(song, quality = "320") {
    try {
        ...
        if (audioData && audioData.url) {
            // 强制使用代理，避免跨域问题
            const proxiedAudioUrl = buildAudioProxyUrl(audioData.url, true);
            const preferredAudioUrl = preferHttpsUrl(audioData.url);

            if (proxiedAudioUrl !== audioData.url) {
                debugLog(`下载链接已通过代理: ${proxiedAudioUrl}`);
            }

            const downloadUrl = proxiedAudioUrl;
            ...
            const fileName = `${song.name}-${artistStr}.${fileExtension}`;

            // 使用 fetch 获取文件内容，确保可以自定义文件名
            showNotification("正在获取文件...");
